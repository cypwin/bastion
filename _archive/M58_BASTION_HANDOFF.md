...
> Comprehensive handoff from SWARM_BRAIN to the BASTION repo for implementing
...
> **SWARM_BRAIN source docs:**
> - Design spec: `docs/superpowers/specs/2026-04-19-m58-smart-offloading-design.md`
> - D0 plan: `docs/superpowers/plans/2026-04-19-m58-d0-batch-llm-calls.md`
> - D1-D5 plans: `docs/superpowers/plans/2026-04-19-m58-d{1..5}-*.md`
...
### Files modified (all in SWARM_BRAIN)
...
**Client-side** (implemented in SWARM_BRAIN, verified in code):
...
These headers are already being sent by SWARM_BRAIN clients. BASTION should
...
Every offloading call in SWARM_BRAIN wraps the Ollama request in a try/except:
...
1. **Start with `X-Task-Complexity` routing** (D4 counterpart) — this is the
   core BASTION change and can be implemented independently of SWARM_BRAIN's
   D1-D3 work
...
All SWARM_BRAIN client-side code is implemented. To test end-to-end:

1. **Commit + merge** D1-D5 code in SWARM_BRAIN (currently uncommitted on main working tree)
...
4. **Run SWARM_BRAIN test suite**: `pytest tests/test_m58_d*.py tests/test_mcp_ollama.py -v`

### What BASTION can do immediately

1. Implement `X-Task-Complexity` routing — client code is ready, header will
   appear on all requests once SWARM_BRAIN D1-D5 are committed
...
## 12. Reference: SWARM_BRAIN File Locations
...
| M58 design spec | `docs/superpowers/specs/2026-04-19-m58-smart-offloading-design.md` |
