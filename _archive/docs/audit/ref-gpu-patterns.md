# SWARM_BRAIN GPU Patterns Adapted for BASTION

> Documents which SWARM_BRAIN patterns were extracted into BASTION and how they
...
**Adaptation**:
- SWARM_BRAIN makes separate subprocess calls per metric (temperature, vram_used, etc.)
...
```python
# SWARM_BRAIN (ollama_client.py:75)
...
**Key difference**: SWARM_BRAIN only protects Python callers using `OllamaClient`.
BASTION protects ALL traffic (including `ollama run`, Claude Code MCP, any HTTP client).

**Streaming**: SWARM_BRAIN's `ollama_client.py` uses `stream=False` (waits for full response).
...
```python
# SWARM_BRAIN pattern (ollama_client.py:418)
...
**Not extracted** (SWARM_BRAIN-specific):
...
- `jury` and `jury_cpu` sections (SWARM_BRAIN-specific roles)

### src/vram_queue.py -> bastion/vram.py (TOCTOU prevention)

**What**: Cross-process VRAM reservation to prevent race conditions.

**SWARM_BRAIN approach**: File-based JSON ledger at `/tmp/swarm_vram_queue.json` with
...
**Trade-off**: SWARM_BRAIN's file lock works without a central process. BASTION requires
...
```python
# SWARM_BRAIN (ollama_client.py:46)
...
## Patterns NOT Adapted (SWARM_BRAIN-specific)
...
When SWARM_BRAIN's GPU safety code changes, check:
...
When BASTION is running, SWARM_BRAIN's internal GPU safety stack becomes redundant
for Ollama traffic. SWARM_BRAIN can simplify to a thin broker status check:
```python
