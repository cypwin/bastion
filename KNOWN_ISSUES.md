...
- **Observed:** 2026-06-11 ~22:02, during the SWARM_BRAIN atlas bulk-extract
  ...
  client-side symptoms recorded in SWARM_BRAIN
  (`scripts/atlas/bulk_extract.py` header comment + S129/S130 session notes).
...
- **Consequence worth flagging:** the failure pushed the batch client to ship
  a proxy-bypass (`ATLAS_OLLAMA_HOST` → direct :11434) — i.e. this bug's
