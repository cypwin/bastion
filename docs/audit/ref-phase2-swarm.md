# Phase 2b — S6: the-batch-client Convergence
...
Implement S6 from `ROADMAP.md`: build the BASTION-side infrastructure for the-batch-client integration — a pip-installable client library, session profiles, and a model intent API. We are NOT modifying the-batch-client itself; that's a separate task for the the-batch-client repo.
